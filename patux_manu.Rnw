...
bestLE12 <- mutate(bestLE12, 
  ...
  flcat = cut(sal, breaks = c(-Inf, quantile(sal, c(0.25, 0.5, 0.75)), Inf), labels = c('Flow 1 (Low)', 'Flow 2', 'Flow 3', 'Flow 4 (High)')), 
...
bestTF16 <- mutate(bestTF16, 
  mo = as.numeric(strftime(date, '%m')), 
  yr = as.numeric(strftime(date, '%Y')), 
  flcat = cut(Q, breaks = c(-Inf, quantile(Q, c(0.25, 0.5, 0.75)), Inf), labels = c('Flow 1 (Low)', 'Flow 2', 'Flow 3', 'Flow 4 (High)')), 
  mocat = cut(mo, breaks = c(-Inf, 3, 6, 9, Inf), labels = c('JFM', 'AMJ', 'JAS', 'OND')), 
  yrcat = cut(yr, breaks = c(-Inf, 1993, 2000, 2007, Inf), 
    labels = c('1986-1993', '1994-2000', '2001-2007', '2008-2014'))
  ) %>% 
...
<<wrtdsvgam, fig.width = 6, fig.height = 6.5, eval = T, echo = F, cache = T, fig.cap = 'Comparison of \\ac{WRTDS} and \\acp{GAM} predictions of \\ac{chla} by different time periods and station (LE1.2, TF1.6).  Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions at each site (low).  Regression lines (thick) and 1:1 replacement lines (thin) are also shown.'>>=
