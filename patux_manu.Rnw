...
\subsection{Performance comparisons and considerations}

Comparisons of alternative statistical models are necessarily defined by the original intent of each model.  Our analyses considered a general overview of each technique that included an assessment of predictive performance for both observed and flow-normalized trends, comparison between models for potential bias and indications of trend, and descriptions of canonical variation related to temporal or flow effects.  The variety of methods for comparing models was chosen because the type of desired information from each model may vary.  As applied to estuaries, both models have the general objective of describing trends from long-term monitoring datasets, whereas alternative applications could focus on hypothesis-testing of changes within discrete time periods.  Different conclusions can also be made from comparing the observed or flow-normalized predictions.  An improvement in predictive performance using \ac{RMSE}, for example, may suggest one model is more advantageous over another if the goal is to reproduce trends, whereas this information may be irrelevant for comparing hypothesis testing.  As such, the results should be interpreted in the context of how a model is applied and the original purpose of its development.  Inferior performance for one metric does not necessarily invalidate an analysis method.  An interpretation of our specific results should consider that the intent was to provide a general overview with a variety of techiques given that we did not explicitly describe the purpose of using each model.   

A general conclusion from our results is that both models provide similar information, both in terms of predictive performance and trends over time in the Patuxent.  The comparisons of \ac{RMSE} consistently showed that \ac{WRTDS} had lower prediction error than \acp{GAM} for all comparisons.  A naive interpretation may suggest that \ac{WRTDS} is a superior model given these results, but we emphasize that the improvement in performance is trivial both in the relative values and fundamental differences between the models.  Prediction error for \acp{GAMs} could easily be improved over \ac{WRTDS} by adjusting model parameters.  This highlights a potential pitfall of using prediction error as a sole metric of performance because the values are sensitive to tuning parameters of each model.  Our intent in developing comparable methods for model development (e.g., cross-validation for \ac{WRTDS}) was to implement an `unbiased' approach for comparing each model.  Both models can be trained with minimal intervention from the analyst using these methods 
% optimal model parameters - similarities and differences with GAMs/WRTDS - see my email to Rebecca on 8/10/15

%The cross-validation method used for \ac{WRTDS} is a generic approach to balance model over- and under-fitting.  It just searches through window width combinations until the minimum error on the test dataset is found, key word being ‘test’.  In theory, cross-validation provides a balance between over-under fitting (fit vs smoothing) because it’s based on the test dataset not used to train the model.  The error on the test dataset was just mean squared error from the predicted values.  I think you can change the function to explicitly include things like a penalty for the smoothing parameter in addition to standard performance measures, but that gets quickly into a realm of unfamiliarity for me.  That being said, the cross-validation I’ve used isn’t just as simple as finding the parameters that minimize overall error, it’s really all about finding the parameters that minimize error on the test dataset.  So it should find a balance, it just may not be as explicit as how it’s done for GAMs.  

%Apologies if this is familiar territory for you, but this was a huge problem I had when I was working on my dissertation.  I was training neural networks (cause I thought they were fancy) to predict habitat quality in MN lakes.  These predictive networks are fit to a dataset using a fixed number of training iterations where the model parameters are changed with each iteration to minimize error.  It’s the same problem we have with WRTDS/GAMs, what optimal smoothing/window width minimizes error?  You can always overfit the model by finding the best parameters that minimize error on the training dataset, but the true measure of a model’s worth is the fit to the test dataset.  I attached a figure that shows the idea for neural networks… you want to stop training the model almost immediately when the error on the test dataset starts to increase with ‘over-fitting’ – that optimal spot.  As I understand it, this is really similar to cross-validation that uses repeated model fits for with training/test data to find the optimal parameters (smoothing or window widths) at which over/under-fitting is balanced.  Conceptually, the figure is the same for our data.  

%Again, I don’t think my cross-validation is as explicit as what’s done with GAMs but I think the concept is similar.  I think this approach could work for an ‘apples to apples’ comparison – i.e., using a similar optimization technique for both models that allows for a more legit comparison.  
...
<<seas, echo = FALSE, eval = T, cache = F, message = F, results = 'hide', fig.width = 6, fig.height = 5, fig.cap = 'Seasonal variation of predictions by station and model.  Points are all model predictions by day of year from 1986 to 2014.  The blue line is a loess (locally estimated) polynomial smooth to characterize the seasonal components.'>>=
