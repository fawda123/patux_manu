...
% what does this mean for the patuxent - not really an emphasis of the paper but interesting trends, e.g., effects of hurricane flushing lasted several years at tf16.  Also seasonal changes at LE12, chlorophyll increasing in cooler months (anecdotally), supported by models?
...
toplo <- rbind(bestLE12, bestTF16) %>% 
  ...
  mutate(mod = factor(mod, levels = c('fits_wrtds', 'fits_gams'), labels = c('WRTDS', 'GAM')))
...
ggplot(toplo, aes(x = fake_date, y = val, group = mod)) + 
  ...
  facet_grid(mod ~ stat, scales = 'free_y') + 
  theme_mine() + 
