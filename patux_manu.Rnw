...
cap_val <- 'Qualitative comparisons of generalized additive models and WRTDS.  Qualities are grouped by ease of use and statistical considerations.  Ease of use qualities are described as good, moderate, or poor and statistical qualitas as yes/no.  The consideration column further describes how the qualities relate to each model.  Qualities were considered as a product provided by each model (or supporting material) or if data characteristics are handled by each model.'
