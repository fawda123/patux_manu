...
% study site map
<<map, fig.height = 8, fig.width = 5, out.width = '0.7\\textwidth', fig.cap = 'Patuxent River estuary with Chesapeake Bay inset. Fixed locations monitored by the Chesapeake Bay Program at monthly frequencies are shown along the longitudinal axis. Salinity regions in the Patuxent for the larger Chesapeake Bay area are also shown (TF = tidal fresh, OH = oligohaline, MH = mesohaline).', echo = FALSE>>=
...
p1 <- ggplot(patux_poly, aes(x = long, y = lat)) + 
  ...
  geom_text(data = pax_meta, aes(x = LONG * 1.0008, y = LAT, label = STATION), size = 5) +
  theme_classic() + 
  ...
  coord_fixed(ratio = 1, xlim = c(-76.8, -76.38), ylim = c(38.27, 38.88))
...
v2 <- viewport(width = 0.55, height = 0.55, x = 0.725, y = 0.68)
print(p1, vp = v1) 
print(p2, vp = v2)
...
<<chlyrmo, fig.width = 6, fig.height = 8, eval = T, echo = F, cache = T, fig.cap = 'Annual and seasonal chlorophyll trends at each monitoring station in the Patuxent River Estuary.  Size and color are proportional medians of ln-chlorophyll-a by year and season categories. See \\cref{fig:map} for station numbers.'>>=
