...
<<chlyrmo, fig.width = 5, fig.height = 5.5, eval = T, echo = F, cache = T, fig.cap = 'Annual and seasonal chlorophyll trends at each monitoring station in the Patuxent River Estuary.  Size and color are proportional medians of ln-chlorophyll-a by year and season categories. See \\cref{fig:map} for station numbers.'>>=
