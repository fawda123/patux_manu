...
rows <- perfTF16[, 1]
cap.val<-'Summary statistics of model performance at TF1.6 for \\ac{WRTDS} and \\acp{GAM} during different time periods.  Overall performance for the entire time series is shown at the top.  Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions of discharge.'
...
# summarize by categories, then combine with overall summary
avediff <- mutate(bestLE12, 
  mo = as.numeric(strftime(date, '%m')), 
  yr = as.numeric(strftime(date, '%Y')), 
  flcat = cut(Q, breaks = c(-Inf, quantile(sal, c(0.25, 0.5, 0.75)), Inf), labels = c('Flow 1 (Low)', 'Flow 2', 'Flow 3', 'Flow 4 (High)')), 
  mocat = cut(mo, breaks = c(-Inf, 3, 6, 9, Inf), labels = c('JFM', 'AMJ', 'JAS', 'OND')), 
  yrcat = cut(yr, breaks = c(-Inf, 1993, 2000, 2007, Inf), 
    labels = c('1986-1993', '1994-2000', '2001-2007', '2008-2014'))
  ) %>% 
...
tab <- form_fun(perfLE12[, -1])
rows <- perfLE12[, 1]
cap.val<-'Summary statistics of model performance at LE1.2 for \\ac{WRTDS} and \\acp{GAM} during different time periods.  Overall performance for the entire time series is shown at the top.  Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions of salinity (labelled as flow where low implies high salinity).'
