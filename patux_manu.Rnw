...
perfLE12 <- select(bestLE12, 
    chla, fits_wrtds, fits_gams, res_wrtds, res_gams, flcat, mocat, yrcat
  ) %>% 
  ...
  summarise(
    ...
    wrtds_rsq = rsq.fun(res_wrtds, chla),
    gam_rsq = rsq.fun(res_gams, chla)
  )
perfLE12 <- rbind(
  with(bestLE12, data.frame(
    cat = '', 
    wrtds_rmse = rmse.fun(res_wrtds),
    gam_rmse = rmse.fun(res_gams),
    wrtds_rsq = rsq.fun(res_wrtds, chla),
...
latex( 
  ...
  cgroup = c('RMSE', '{\\bf \\textit{R}\\textsuperscript{2}}'),
  n.cgroup = c(2, 2),
  rowname = rows,
  colheads = rep(c('WRTDS', 'GAM'), 2),
  label = 'tab:perfLE12'
  )
...
summpred <- rbind(diffTF16, diffLE12) %>% 
  group_by(cat, stat) %>% 
  summarise(
    aved = ave.fun(fits_wrtds, fits_gams)
  ) %>%
  mutate(stat = paste0('pred', stat)) %>% 
  spread('stat', 'aved')
...
summpred <- rbind(
  data.frame(
    cat = '', 
    predLE1.2 = with(bestLE12, ave.fun(fits_wrtds, fits_gams)), 
    predTF1.6 = with(bestTF16, ave.fun(fits_wrtds, fits_gams))
    ),
  summpred
)

# summmarize by categories, normalized 
summnorm <- rbind(diffTF16, diffLE12) %>% 
  group_by(cat, stat) %>% 
  summarise(
    aved = ave.fun(norm_wrtds, norm_gams)
  ) %>% 
  mutate(stat = paste0('norm', stat)) %>% 
  spread('stat', 'aved') 

# add summary for whole time series, normalized
summnorm <- rbind(
  data.frame(
    cat = '', 
    normLE1.2 = with(bestLE12, ave.fun(norm_wrtds, norm_gams)), 
    normTF1.6 = with(bestTF16, ave.fun(norm_wrtds, norm_gams))
    ),
  summnorm
)

totab <- left_join(summpred, summnorm, by = 'cat')

# latex table
tab <- form_fun(totab[, -1])
rows <- totab[, 1]
cap.val <- 'Average differences (\\%) between \\ac{WRTDS} and \\acp{GAM} at each station during different time periods.  Differences for predicted and flow-normalized results from each model are provided.  Overall differences for the entire time series is shown at the top.  Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions of discharge.'
...
latex( 
  ...
  n.cgroup = c(2, 2),
  rowname = rows,
  colheads = rep(c('Pred', 'Norm'), 2),
  label = 'tab:avediff'
  )

@

% regression fit between WRTDS, GAMs, predictions
<<eval = T, echo = F, cache = T>>=
data(bestLE12)
data(bestTF16)

diffLE12 <- select(bestLE12, 
    fits_wrtds, fits_gams, flcat, mocat, yrcat
  ) %>% 
  gather(key = 'cats', value = 'cat', flcat:yrcat) %>% 
  mutate(cat = factor(cat, levels = c('1986-1993', '1994-2000', '2001-2007', '2008-2014', 'JFM', 'AMJ', 'JAS', 'OND', 'Flow 1 (Low)', 'Flow 2', 'Flow 3', 'Flow 4 (High)')),
    stat = 'LE1.2'
...
# summmarize by categories, predictions
pred <- rbind(diffTF16, diffLE12) %>% 
  group_by(cat, stat)

intpred <- summarise(pred,
    int = lm.fun(fits_wrtds, fits_gams, origin = FALSE)[1]
  ) %>%
  mutate(stat = paste0('int', stat)) %>% 
  spread('stat', 'int')

slopred <- summarise(pred,
    slo = lm.fun(fits_wrtds, fits_gams, origin = FALSE)[2]
  ) %>%
  mutate(stat = paste0('slo', stat)) %>% 
  spread('stat', 'slo')

sloorgpred <- summarise(pred,
    sloorg = lm.fun(fits_wrtds, fits_gams, origin = TRUE)
  ) %>%
  mutate(stat = paste0('sloorg', stat)) %>% 
  spread('stat', 'sloorg')

pred <- left_join(intpred, slopred, by = 'cat') %>% 
  left_join(., sloorgpred,  by = 'cat')

# add summary for whole time series, predictions
totab <- rbind(
  data.frame(
    cat = '', 
    intLE1.2 = with(bestLE12, lm.fun(fits_wrtds, fits_gams, origin = FALSE))[1], 
    intTF1.6 = with(bestTF16, lm.fun(fits_wrtds, fits_gams, origin = FALSE))[1],
    sloLE1.2 = with(bestLE12, lm.fun(fits_wrtds, fits_gams, origin = FALSE))[2], 
    sloTF1.6 = with(bestTF16, lm.fun(fits_wrtds, fits_gams, origin = FALSE))[2],
    sloorgLE1.2 = with(bestLE12, lm.fun(fits_wrtds, fits_gams, origin = TRUE)), 
    sloorgTF1.6 = with(bestTF16, lm.fun(fits_wrtds, fits_gams, origin =TRUE))
    ),
  pred
)
...
latex( 
  ...
  rgroup = c('All', 'Annual', 'Seasonal', 'Flow'),
  n.rgroup = c(1, rep(4, 3)),
  cgroup = c('Intercept', 'Slope', "Slope$'$"),
  n.cgroup = rep(2, 3),
  rowname = rows,
  colheads = rep(c('LE1.2', 'TF1.6'), 3),
  label = 'tab:regpred'
  )

@

% regression fit between WRTDS, GAMs, flow-normalized predictions
<<eval = T, echo = F, cache = T>>=
data(bestLE12)
data(bestTF16)

diffLE12 <- select(bestLE12, 
    norm_wrtds, norm_gams, flcat, mocat, yrcat
  ) %>% 
  gather(key = 'cats', value = 'cat', flcat:yrcat) %>% 
  mutate(cat = factor(cat, levels = c('1986-1993', '1994-2000', '2001-2007', '2008-2014', 'JFM', 'AMJ', 'JAS', 'OND', 'Flow 1 (Low)', 'Flow 2', 'Flow 3', 'Flow 4 (High)')),
    stat = 'LE1.2'
...
# summmarize by categories, predictions
norm <- rbind(diffTF16, diffLE12) %>% 
  group_by(cat, stat)

intnorm <- summarise(norm,
    int = lm.fun(norm_wrtds, norm_gams, origin = FALSE)[1]
  ) %>%
  mutate(stat = paste0('int', stat)) %>% 
  spread('stat', 'int')

slonorm <- summarise(norm,
    slo = lm.fun(norm_wrtds, norm_gams, origin = FALSE)[2]
  ) %>%
  mutate(stat = paste0('slo', stat)) %>% 
  spread('stat', 'slo')

sloorgnorm <- summarise(norm,
    sloorg = lm.fun(norm_wrtds, norm_gams, origin = TRUE)
  ) %>%
  mutate(stat = paste0('sloorg', stat)) %>% 
  spread('stat', 'sloorg')

norm <- left_join(intnorm, slonorm, by = 'cat') %>% 
  left_join(., sloorgnorm,  by = 'cat')

# add summary for whole time series, flow-normalized predictions
totab <- rbind(
  data.frame(
    cat = '', 
    intLE1.2 = with(bestLE12, lm.fun(norm_wrtds, norm_gams, origin = FALSE))[1], 
    intTF1.6 = with(bestTF16, lm.fun(norm_wrtds, norm_gams, origin = FALSE))[1],
    sloLE1.2 = with(bestLE12, lm.fun(norm_wrtds, norm_gams, origin = FALSE))[2], 
    sloTF1.6 = with(bestTF16, lm.fun(norm_wrtds, norm_gams, origin = FALSE))[2],
    sloorgLE1.2 = with(bestLE12, lm.fun(norm_wrtds, norm_gams, origin = TRUE)), 
    sloorgTF1.6 = with(bestTF16, lm.fun(norm_wrtds, norm_gams, origin =TRUE))
    ),
  norm
)
...
tab <- totab[, -1]
rows <- totab[, 1]
cap.val <- "Regression fits comparing \\ac{WRTDS} and \\acp{GAM} flow-normalized predictions at each station during different time periods.  Fits for the entire time series are shown at the top.  The second slope column (Slope$'$) describes a regression that was forced through the origin.  Values in bold-italic are those where the intercept estimate was significantly different from zero or the slope estimate was significantly different from one.   Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions of discharge."
...
% study site map
<<map, fig.height = 7, fig.width = 5, out.width = '0.7\\textwidth', fig.cap = 'Patuxent River estuary with Chesapeake Bay inset. Fixed locations monitored by the Chesapeake Bay Program at monthly frequencies are shown along the longitudinal axis with distance from the mouth (km).  Study sites are in bold. Salinity regions in the Patuxent for the larger Chesapeake Bay area are also shown (TF = tidal fresh, OH = oligohaline, MH = mesohaline).', echo = FALSE>>=
...
<<wrtdsvgam, fig.width = 6, fig.height = 6.5, eval = T, echo = F, cache = T, fig.cap = 'Comparison of \\ac{WRTDS} and \\acp{GAM} predictions of \\ac{chla} at each station (LE1.2, TF1.6) and different time periods.  Time periods are annual groupings every seven years (top), seasonal groupings by monthly quarters (middle), and flow periods based on quantile distributions from the discharge record (low).  Regression lines (thick black) and 1:1 replacement lines (thin grey) are also shown.'>>=
...
p1 <- ggplot(toplo[toplo$cats %in% 'yrcat', ], aes(x = fits_wrtds, y = fits_gams)) + 
  geom_abline(intercept = 0, slope = 1, colour = 'grey') + 
  geom_point(alpha = 0.7, colour = cols[1]) + 
  facet_grid(stat ~ cat) +
...
p2 <- ggplot(toplo[toplo$cats %in% 'mocat', ], aes(x = fits_wrtds, y = fits_gams)) + 
  geom_abline(intercept = 0, slope = 1, colour = 'grey') + 
  geom_point(alpha = 0.7, colour = cols[1]) + 
  scale_x_continuous(limits = c(0, 4)) + 
  scale_y_continuous(limits = c(0, 4)) + 
...
p3 <- ggplot(toplo[toplo$cats %in% 'flcat', ], aes(x = fits_wrtds, y = fits_gams)) + 
  geom_abline(intercept = 0, slope = 1, colour = 'grey') + 
  geom_point(alpha = 0.7, colour = cols[1]) + 
  scale_x_continuous(limits = c(0, 4)) + 
  scale_y_continuous(limits = c(0, 4)) + 
...
grid.arrange(p1, p2, p3, left = textGrob(ylab, rot = 90), bottom = textGrob(xlab))
