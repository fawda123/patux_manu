...
<<eval = F>>=
# need to redo this with actual 'best' mods, this is just an example
...
tomod <- select(bestLE12, date, dec_time, chla, sal) %>% 
  mutate(
    doy = as.numeric(strftime(date, '%j'))
  )

gam1 <- gam(chla~te(dec_time,doy,sal,bs=c("tp","cc","tp")), data = tomod, knots=list(doy=c(1,366)))

dynagam(gam1, tomod)

data(bestLE12_wrtds)

dynaplot(bestLE12_wrtds)
