...
\noindent KEY TERMS: chlorophyll, estuary, generalized additive models, Patuxent River Estuary, trend analysis, weighted regression
...
\subsubsection*{\hspace*{0.5in}Weighted Regression on Time, Discharge, and Season.}
...
\subsubsection*{\hspace*{0.5in}Generalized Additive Models.}
...
\subsubsection*{\hspace*{0.5in}Selection of model parameters.}
...
We thank Jim Hagy, Bob Hirsch, Jennifer Keisman, Elgin Perry, and Jeremy Testa for valuable discussions that improved the analysis.  We thank the Chesapeake Bay Program and Maryland Department of Natural Resources for providing data.
...
labs <- c('Biological \\ac{chla}', 'No flow', 'Constant flow', 'Increasing flow')
