...
p1 <- p + facet_grid(stat ~ yrcat)
p2 <- p + facet_grid(stat ~ mocat)
p3 <- p + facet_grid(stat ~ flcat)
...
theme_mine <- function (base_size = 12, base_family = "") {
  ...
  theme(
    ...
    legend.background = element_rect(fill='transparent', 
      colour = NA),
    strip.background = element_rect(fill = 
        scales::alpha(cols[length(cols)], 0.5)),
    legend.key = element_rect(fill = 'transparent', 
      colour = NA)
    )   
}
...
toplo <- select(sims_day, date, lnchla_noQ, sim1, sim2, sim3) %>% 
  tidyr::gather('variable', 'value', -date)
toplo2 <- select(sims_mos, date, lnchla_noQ, sim1, sim2, sim3) %>% 
  tidyr::gather('variable', 'value', -date) 

# factor labels as expressions
# labs <- c(expression(italic(Chl [bio])), expression(Constant~flow), expression(No~flow), 
#   expression(Increasing~flow))
labs <- c('Biological chlorophyll', 'Constant flow', 'No flow', 'Increasing flow')
