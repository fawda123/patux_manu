...
\author{
  ...
  \vspace{1in} 
  \\ Version Date:   Wed Aug 5 10:25:40 2015 -0500
	}
\date{}
\maketitle
\end{singlespace}
...
\begin{equation}
\textrm{Average difference} = \left(\frac{\sum\limits_{i = 1}^n \hat{Chl}_{WRTDS,\,i} - \sum\limits_{i = 1}^n \hat{Chl}_{GAM,\,i}}{\sum\limits_{i = 1}^n \hat{Chl}_{WRTDS,\,i}}\right) * 100
\end{equation}
where $\hat{Chl}_{WRTDS,\,i}$ and $\hat{Chl}_{GAM,\,i}$ are the predicted values of \ac{chla} for each model at observation $i$.

Statistical comparisons were conducted for the entire time series at each station to evaluate overall performance.  Different time periods were also evaluated to identify potential temporal variation in results, which included a comparison of results by annual and seasonal aggregations and periods with different levels of flow.  Annual and seasonal aggregations between models were evaluated as shown \cref{fig:chlyrmo}, whereas models were evaluated at different levels of flow defined by the quartile distributions (min--25\%, 25\%--median, median--75\%, and 75\%--max) of either observed flow (TF1.6) or salinity (LE1.2).  Flow-normalized time series were compared similarly but only between the models because the true flow-independent component of the observed data is not known and can only be empirically estimated.  As described below, an evaluation of the flow-normalized data was primarily accomplished using simulated datasets with known components that were independent of discharge.  
...
\caption[Patuxent River estuary with Chesapeake Bay inset]{Patuxent River estuary with Chesapeake Bay inset. Fixed locations monitored by the Chesapeake Bay Program at monthly frequencies are shown along the longitudinal axis with distance from the mouth (km).  Study sites are in bold. Salinity regions in the Patuxent for the larger Chesapeake Bay area are also shown (TF = tidal fresh, OH = oligohaline, MH = mesohaline).}\label{fig:map}
\end{figure}
...
{\centering \includegraphics[width=\maxwidth]{figs/chlyrmo-1} 

}

\caption{Annual and seasonal chlorophyll trends at each monitoring station in the Patuxent River Estuary.  Size and color are proportional medians of ln-chlorophyll-a by year and season categories. See \cref{fig:map} for station numbers.}\label{fig:chlyrmo}
\end{figure}
...
\caption{Predicted chlorophyll from generalized additive models and weighted regression for LE1.2 and TF1.6 stations on the Patuxent River estuary.  \cref{fig:predmo} shows results at monthly time steps and \cref{fig:predann} shows results averaged by year.  Values in blue are model predictions and values in yellow are flow-normalized predictions.}
\label{fig:pred}
\end{figure}
