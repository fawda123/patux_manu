...
\begin{singlespace}
\title{{\bf {\Large Numeric contrasts and recommendations for use of two statistical models of water quality in tidal waters\footnote{Version Date:   Mon Jun 20 18:43:56 2016 -0500}}}}
...
%latex.default(tab, file = "", rowlabel = "Quality", caption = cap_val,     caption.loc = "top", rgroup = categ, n.rgroup = c(4, 5),     rowname = qualt, colheads = c("Consideration\\textsuperscript{1}",         "\\ac{GAM}", "\\ac{WRTDS}"), label = "tab:qualcomp",     insert.bottom = foots)%
\begin{table}[!tbp]
\caption{Qualitative comparisons of generalized additive models and WRTDS.  Qualities are grouped by ease of use and statistical considerations.  Ease of use qualities are described as good, moderate, or poor and statistical qualities as yes/no.  The consideration column further describes the qualities either as a product provided by each model (or supporting material) or if data characteristics are addressed by each model.\label{tab:qualcomp}} 
\begin{center}
\begin{tabular}{llll}
\hline\hline
