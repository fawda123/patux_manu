...
\author{
  ...
  \vspace{1in} 
  \\ Version Date:   Fri Aug 14 13:44:20 2015 -0500
	}
\date{}
\maketitle
\end{singlespace}
...
The predicted \ac{chla} from each model generally followed patterns in observed \ac{chla} from 1986 to 2014 (\Cref{fig:pred}).  For each model, seasonal minima at LE1.2 were observed in late Fall, typically in December, whereas maximum \ac{chla} was observed in an early spring bloom.  A secondary, smaller seasonal peak was also observed in late summer as a result of bottom-layer regeneration and upward nutrient transport \citep{Testa08a}.  A similar secondary bloom was not observed in the upper estuary at TF1.6, where the predicted values of each model suggested a dominant unimodal pattern within each growing season. TF1.6 seasonal maxima were more prolonged than those at LE1.2 with elevated \ac{chla} starting in mid-summer and ending by early Fall, whereas an abrupt minimum was observed in December and January.  Visual evaluation of seasonal trends for each model suggested the results were similar, although \ac{WRTDS} predictions had slightly better fits at the extreme ends of the distribution of \ac{chla}.  Overall, both models provided predictions that more adequately described the range of \ac{chla} at TF1.6 as compared to LE1.2 where observed values lower or higher than the predicted values were more common.  However, quantitative summaries of model fit by site indicated that \ac{RMSE} between sites were similar with errors ranging from a minimum of 0.51 at LE1.2 for \ac{WRTDS} predictions and a maximum of 0.56 at TF1.6 for \ac{GAM} predictions (\Cref{tab:perftoobs}).  

Comparisons of fit between models showed that \ac{WRTDS} had lower \ac{RMSE} for the overall time series and all time periods (annual, seasonal, and flow) that were evaluated, regardless of site.  Fit by different time periods generally showed agreement between methods during periods when performance was relatively high or low.  For LE1.2, models had the worst fit during the 2001-2007 annual aggregations, the April-May-June seasonal aggregations, and periods of high flow.  For TF1.6, models had the worst fit during the 1994-2000 annual aggregation, whereas the worst fit by seasonal and flow aggregations varied.  Model fit was the worst during the October-November-December seasonal aggregation for \acp{GAM}, whereas fit was the worst for the April-May-June aggregation for \ac{WRTDS}.  Maximum error for each model at TF1.6 followed similar patterns for the flow aggregations, with the worst performance for \acp{GAM} occuring during high flow and the worst performance for \ac{WRTDS} occuring during the second quartile for flow.  In general, model performance was linked to flow such that fit decreased during periods of high flow, either during seasonal or inter-annual periods of flow minima.    

Results as annual aggregations suggested that \ac{chla} patterns between years have not been constant and are considerably different between sites (\cref{fig:pred}).  Both models showed a gradual and consistent increase in \ac{chla} at LE1.2, with values increasing by approximately 1.5 \mugl from 1986 to 2014.  Predictions at TF1.6 did not show a similar increase from the beginning to the end of the time series, although a dramatic decrease from approximately 12 \mugl to 6 \mugl from 2000 to 2006 was observed.  Chlorophyll values returned to the approximate site average by 2014.  Flow-normalized trends at each site provided an indication of discharge or salinity effects between sites.  Apparent 
...
Visual eval of trends from plot \cref{fig:pred}
\Cref{tab:statsum}, \Cref{tab:perftoobs}, 

\subsection{Comparison of model predictions}

\Cref{tab:perfbtw}, \Cref{tab:regprdnrm}, \Cref{tab:trendsLE12}, \Cref{tab:trendsTF16}, \Cref{fig:pred}, \Cref{fig:regprdnrm}
