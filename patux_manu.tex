...
\author{
  ...
  \vspace{1in} 
  \\ Version Date:   Fri Jul 24 09:28:47 2015 -0500
	}
\date{}
\maketitle
\end{singlespace}
...
The interpretation of enviromental trends can have widespread implications for the management of environmental resources and can serve as a basis for understanding ecological factors that mediate system dynamics.  An accurate interpretation of trends is highly dependendent on the method of analysis, and more importantly, the ability of the method to consider effects of multiple drivers on response endpoints that may be particular to the system of interest.  The need to interpret potential impacts of nutrient pollution has historically, and continues to be, a priority issue for managing aquatic resources \citep{Nixon95}, particularly for estuaries that serve as focal points of human activities and receiving bodies for upstream hydrologic networks \citep{Paerl14}.  Common response endpoints for assessing eutrophication effects in estuaries have included seagrass growth patterns \citep{Steward07}, frequency and magnitude of oxygen depletion in bottom waters \citep{Paerl06}, and trophic network connectivity \citep{Powers05}.  Additionally, routine monitoring of chlorophyll concentrations can provide an informative measure of the release of phytoplankon communities from nutrient limitation with increasing eutrophication.  Chlorophyll time series have been collected for decades in tidal systems (e.g., Tampa Bay, \citet{TBEP11}; Chesapeake Bay, \citet{Harding94}; datasets cited in \citet{Monbet92,Cloern10}), although the interpration of trends in observed data has been problematic given the variability of high-resolution time series data. Identifying the response of chlorophyll endpoints to different drivers, such as management actions or increased pollutant loads, can be complicated by natural variation from freshwater inflows or tidal exchange with oceanic outflows.  Seasonal and spatial variability of chlorophyll dynamics (see \citet{Cloern96}) can further complicate trend evaluation, such that relatively simple analysis methods (e.g., ) have insufficiently described variation in long-term datasets.  Quantitative tools that can characterize chlorophyll response independent of variation from confounding variables are needed to increase the certainty of conclusions about factors that influence ecosystem condition.

% WRTDS and GAMs as novel methods
Recent applications of novel statistical methods to describe water quality dynamics have shown promise in estuaries, specifically \ac{WRTDS} and \acp{GAM}.  The \ac{WRTDS} method was initially developed to describe water quality trends in rivers \citep{Hirsch10,Hirsch14} and has recently been adapted to describe chlorophyll trends in tidal waters \citep{Beck15}.  A defining characteristic of the \ac{WRTDS} model is a weighting scheme that fits a continuous set of parameters to the time series by considering the relative influence of location in the record and contextual flow inputs to the period of interest.  To date, the \ac{WRTDS} model has been used to model pollutant delivery from tributary sources to Chesapeake Bay \citep{Hirsch10,Moyer12,Zhang13}, Lake Champlain \citep{Medalie12}, and the Mississippi River \citep{Sprague11}.  A comparison to an alternative regression-based model for evaluating nutrient flux, ESTIMATOR, suggested that \ac{WRTDS} can produce more accurate trend estimates \citep{Moyer12}. \acp{GAM} are a more generic statistical model that can be used to describe variation in a response variable as a sum of smoothing functions for different predictors \citep{Hastie90,Wood06}.  In applications to water quality time series, \acp{GAM} are similar to \ac{WRTDS} in that variable effects through time of different drivers can be described in the context of seasonal or annual changes.  Application of \acp{GAM} to describe eutrophication endpoints in tidal waters have not been as extensive as \ac{WRTDS}, although exploratory analyses have suggested that results are comparable.  Moreoever, \acp{GAM} are particularly appealing because they are less computationally intense and provide more accessible estimates of model uncertainty than \ac{WRTDS}.  Despite the potential for both approaches to guide management applications, the relative merits of each have not been rigorously evaluated.  Quantitative comparisons that describe the accuracy empirical descriptions and the desired products could inform the use of each model to describe long-term changes in ecosystem characteristics.

% goals/objectives
The goal of this study is to provide an empirical description of the relative abilities of \ac{WRTDS} and \acp{GAM} to describe long-term changes in time series of eutrophication response endpoints in tidal waters.  A long-term time series of monthly chlorophyll observations from the Patuxent River Estuary is used as a common dataset for evaluating each model.  The Patuxent Estuary is a well-studied tributary of the Chesapeake Bay system that has been monitored for thirty years with fixed stations along the longitudinal axis.  Two stations were chosen as representative time series that differed in the relative contributions of watershed inputs and influences from the mainstem of the Chesapeake, in addition to known historical events that have impacted water quality in the estuary.  This provided a unique opportunity to evaluate each model's ability to interpret effects of different drivers on primary production.  The specific objectives of the analysis were to \begin{inparaenum}[1\upshape)]
